...
class Client:
	...
	# def recvpubkey(self):
	# 	responseData = bytes.fromhex(self.receive()).decode()
	# 	print('respdat', responseData)
	# 	if "PUB@@" in responseData[:5]:
	# 		self.holder_public_key = serialization.load_pem_public_key(
	# 			bytes.fromhex(responseData[5:].decode()),
	# 			backend=default_backend(),
	# 		)
	# 		self.shared_key = self.private_key.exchange(self.holder_public_key)
	# 		print("pubkey diterima")
	# 	else:
	# 		self.client_socket.close()
	# 		exit()
